...
# Script de otimização total para mobile
echo "Iniciando otimização focada em dispositivos móveis..."

# 1. Criar um arquivo JS exclusivo para correções mobile
cat > js/mobile-complete-fix.js << 'EOL'
/**
...
# 2. Adicionar o script ao index.html
if ! grep -q "js/mobile-complete-fix.js" index.html; then
  echo "Adicionando script de otimização mobile ao index.html..."
  
  # Verificar se já existem outros scripts de correção e removê-los
  sed -i '/js\/mobile-fix.js/d' index.html
  
  # Adicionar novo script na posição correta - após todos os outros scripts
  sed -i '/<script src="js\/script.js"><\/script>/a \    <script src="js/mobile-complete-fix.js"></script>' index.html
fi

echo "Otimização mobile completa aplicada!"
echo "Por favor, teste no seu dispositivo móvel para verificar:"
echo "1. Se os filtros estão funcionando corretamente"
echo "2. Se as imagens estão abrindo na galeria e não em nova janela"
echo "3. Se a navegação entre as fotos está na ordem correta"
echo "Se tudo estiver funcionando, execute 'firebase deploy' para aplicar no servidor."