...
# Script de correção para problemas em dispositivos móveis
echo "Iniciando correção para problemas em dispositivos móveis..."

# 1. Criar script de correção específico para móvel
cat > js/mobile-fix.js << 'EOL'
/**
...
if ! grep -q "js/mobile-fix.js" index.html; then
  echo "Adicionando script de correção mobile ao index.html..."
  sed -i '/<script src="js\/script.js"><\/script>/a \    <script src="js/mobile-fix.js"></script>' index.html
fi

echo "Correção aplicada! Os problemas de dispositivos móveis devem estar resolvidos."
echo "Atualize a página em um dispositivo móvel e verifique se os filtros e a galeria estão funcionando corretamente."
echo "Se tudo estiver funcionando, execute 'firebase deploy' para atualizar o site."