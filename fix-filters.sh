...
# Script de correção para o problema de filtros
echo "Iniciando correção específica para os filtros..."

# Criar um script JS focado apenas nos filtros
cat > js/filter-fix.js << 'EOL'
/**
...
if ! grep -q "js/filter-fix.js" index.html; then
  echo "Adicionando script de correção de filtros ao index.html..."
  
  # Certificar-se de que o script é o último a ser carregado
  if grep -q "js/mobile-complete-fix.js" index.html; then
    # Se o script mobile-complete-fix.js existir, adicione depois dele
    sed -i '/js\/mobile-complete-fix.js/a \    <script src="js/filter-fix.js"></script>' index.html
  else
    # Caso contrário, adicione após o script.js
    sed -i '/<script src="js\/script.js"><\/script>/a \    <script src="js/filter-fix.js"></script>' index.html
  fi
fi

echo "Correção de filtros aplicada com sucesso!"
echo "Por favor, teste no seu dispositivo móvel para verificar se os filtros estão funcionando."
